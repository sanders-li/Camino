...
class RouteFinder:
    ...
    def print_solution(self):
        ...
        for day in range(self.data['length_of_stay']):
            ...
            while not self.routing.IsEnd(index):
                ...
                load = self.solution.Value(load_var) - previous_load
                if not first:
                    plan_output += '\t'
            ...
            load = self.solution.Value(load_var) - previous_load
            travel_time = self.solution.Min(time_var) - previous_time - previous_load
            plan_output += ' [Location {0}: {1}] | Time({2},{3}) | Travel Time From Previous: {4} | Time Spent Here: {5}\n'.format(
                self.manager.IndexToNode(index),
                self.data['names'][self.manager.IndexToNode(index)],
                self.solution.Min(time_var), 
                self.solution.Max(time_var),
                travel_time, 
                self.solution.Value(load_var))
            day_travel_times.append(Travel(previous_index, self.manager.IndexToNode(index), travel_time))
    ...
    def visualize(self):
        ...
        hotel_location = self.data['locations'][0]
        locations = self.data['locations'][1:]
        plt.scatter(hotel_location.lng, hotel_location.lat, zorder=2, alpha=1, c='r', s=100)
        plt.scatter([point.lng for point in locations], [point.lat for point in locations], zorder=1, alpha=0.5, c='b', s=100)
        ax.text(hotel_location.lng, hotel_location.lat-0.003, '{}: {}'.format(0, self.data['names'][0]), ha='center', va='center', bbox=styling)

        for i, point in enumerate(locations):
            ax.text(point.lng, point.lat+0.003, '{}: {}'.format(i+1, self.data['names'][i]), ha='center', va='center', bbox=styling)
